...
@section('breadcumb')
<div class="row">
    <div class="col-sm-6"><h3 class="mb-0">Laporan Bulan Sekarang</h3></div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-end">
            <li class="breadcrumb-item"><a href="{{ route('bulansekarang') }}">Home</a></li>
            <li class="breadcrumb-item active" aria-current="page">Laporan Bulan Sekarang</li>
        </ol>
    </div>
</div>
@endsection
...
<script>
    const chartData = JSON.parse('{!! json_encode($chardatapemasukan) !!}');
    ...
    const chartDatae = JSON.parse('{!! json_encode($chardatapengeluaran) !!}');
        ...
        options: {
            ...
            onClick: function (evt, elements) {
                if (elements.length > 0) {
                    ...
                    const url = `{{ url('/bulansekarang/kategori') }}/${encodeURIComponent(category)}?type=income`;
        ...
        options: {
            ...
            onClick: function (evt, elements) {
                if (elements.length > 0) {
                    const index = elements[0].index;
                    const category = this.data.labels[index];
                    const url = `{{ url('/bulansekarang/kategori') }}/${encodeURIComponent(category)}?type=expense`;
                    window.location.href = url;
                }
            }
        }
