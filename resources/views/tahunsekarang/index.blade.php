...
<script>
  ...
  document.getElementById('incomePieChart').onclick = function(evt) {
    const points = incomePieChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true);
    if (points.length) {
      const index = points[0].index;
      const category = incomePieChart.data.labels[index];
      const encoded = encodeURIComponent(category);
      const url = `{{ url('/tahunsekarang') }}/${year}/kategori/${encoded}?type=pemasukan`;
      window.location.href = url;
    }
  };

  document.getElementById('expensePieChart').onclick = function(evt) {
    const points = expensePieChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true);
    if (points.length) {
      const index = points[0].index;
      const category = expensePieChart.data.labels[index];
      const encoded = encodeURIComponent(category);
      const url = `{{ url('/tahunsekarang') }}/${year}/kategori/${encoded}?type=pengeluaran`;
      window.location.href = url;
    }
  };
</script>
